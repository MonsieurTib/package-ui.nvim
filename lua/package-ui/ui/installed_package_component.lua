...
function M.setup_key_mappings()
  ...
  api.nvim_buf_set_keymap(component_state.buf, "n", "u", "", {
    callback = function()
      M.trigger_package_uninstall()
    end,
    desc = "Uninstall selected package",
  })
...
function M.trigger_package_uninstall()
...
function M.updateContent(lines)
  ...
  for i, line in ipairs(lines or {}) do
    -- Look for update indicator pattern (→ version)
    local start_pos, end_pos = string.find(line, " → .+")
    if start_pos and end_pos then
      -- Highlight the arrow and version part
      api.nvim_buf_add_highlight(
        component_state.buf,
        -1,
        "PackageUiUpdateIndicator",
        i - 1,
        start_pos - 1,
        end_pos
      )
...
function M.selectPackage(package_name)
  if not M.isValid() or not package_name then
    return false
  end
  ...
  for i, line in ipairs(lines) do
    local package_line_name = line:match("^([^@]+)")
    if package_line_name then
      package_line_name = package_line_name:gsub("^%s+", ""):gsub("%s+$", "")

      if package_line_name == package_name then
...
function M.has_results()
  ...
  if
      first_line:match("^No ")
      or first_line:match("^loading ")
      or first_line:match("^Loading ")
      or first_line:match("^Uninstalling ")
      or first_line:match("^Installing ")
      or first_line:match("^Failed to ")
      or first_line:match("^Service not available")
      or first_line:match("^Supported package managers:")
  then
    return false
  end
